...
def main():
    ...
    match model_type:
    case "LlamaCpp":
        llm = LlamaCpp(model_path=model_path, max_tokens=model_n_ctx, n_batch=model_n_batch, callbacks=callbacks, verbose=False, temperature=model_temperature)  # Assuming LlamaCpp accepts a temperature argument
    case "GPT4All":
        llm = GPT4All(model=model_path, max_tokens=model_n_ctx, backend='gptj', n_batch=model_n_batch, callbacks=callbacks, verbose=False, temperature=model_temperature)  # Assuming GPT4All accepts a temperature argument
    case _default:
